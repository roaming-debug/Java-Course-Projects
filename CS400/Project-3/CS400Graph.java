...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        if(!containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("start data or end data cannot be found.");
        }
        Vertex startV = vertices.get(start);
        Vertex endV = vertices.get(end);
        Path startPath = new Path(startV);
        visited.put(start, startPath);
        for (Vertex v : vertices.values()) {
            if(!v.data.equals(startV.data)) {
                Path pathPlaceHolder = new Path(v);
                visited.put(v.data, pathPlaceHolder);
            }
        }
        Path path = new Path(startV);
        for (Edge e : startV.edgesLeaving) {
            Path pathLeaving = new Path(path, e);
            frontier.add(pathLeaving);
        }
        while(!frontier.isEmpty()) {
            Path currShortestPath = frontier.poll();
            Vertex toBeVisitedV = currShortestPath.end;
            visited.put(toBeVisitedV.data, currShortestPath);
//            printVisited(visited);
            for (Edge e : toBeVisitedV.edgesLeaving) {
                if(!visited.get(e.target.data).start.data.equals(e.target.data) || e.target.data.equals(start)) continue;
                Path pathLeaving = new Path(currShortestPath, e);
                // update frontier
                boolean foundShorterPath = false;
                for (Path p : frontier) {
                    if (p.end.equals(e.target)) {
                        if(p.distance > pathLeaving.distance) {
                            frontier.remove(p);
                        }
                        if (p.distance <= pathLeaving.distance) {
        ...
        if(visited.get(end).start.data.equals(end)) throw new NoSuchElementException("No paths have been found.");
        return visited.get(end);
    }

    /**
     * Print Visited hashtable
     * @param visited visited hashtable
     */
    private void printVisited(Hashtable<T, Path> visited) {
        for (T data : visited.keySet()) {
            System.out.print(data + " ");
            System.out.print(visited.get(data).dataSequence + " " + visited.get(data).distance +"\n");
        }
        System.out.println('\n');
    }
    /**
